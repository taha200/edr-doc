...
socket.on('Sent',this.onReceiveMessage)
    ...
    stop = async () => {
          ...
          var obje={
            ...
            image:'',
            createdAt: new Date(),
            senderID:this.state.chatData.DocID,
            audio:{
              file:audioFile,
              isPlay:false
            },
    ...
    DoVideo=()=>{
      var newArr = this.state.massages;
      var abc=this.messageIdGenerator();
    ...
    <View style={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center'}}>
        <Icon type="antdesign" name="close" color="white" onPress={()=>this.props.navigation.goBack(null)} />
          <View style={{display:'flex',flexDirection:'row',alignItems:'center',width:'100%'}}>
              ...
              <View style={{View:'20%'}}>
      <Text style={{fontSize:20,color:'white'}}>{this.state.chatData.Patientname}</Text>
              </View>
              <View style={{width:'55%',alignItems:"flex-end",flexDirection:"row",justifyContent:"space-between"}}>
                <View>

                </View>
                <View style={{flexDirection:"row"}}>
              {/* <Icon type="ionicons" name="call" color="#3265b0" raised onPress={()=>this.DoVoice()} /> */}
              <Icon type="ionicons" name="videocam" color="#3265b0" raised onPress={()=>{this.DoVideo()}} />
              </View>
              ...
              <View style={{padding:10,flex:1}}>
                {
                  this.state.massages.map((message,i)=>{
                      if(message.senderID!==this.state.chatData.DocID)
                      {
                        return(
                          <View>
                            {
                                ...
                                <View style={{backgroundColor:'#3265b0',padding:10,paddingBottom:10,borderRadius:15,alignSelf:'flex-end'}}>
                                    {
                                      ...
                                      : message.join !== '' ? <Text style={{color:'white'}}>{"You did a Call"}</Text>
